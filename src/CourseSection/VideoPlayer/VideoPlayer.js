...
class VideoPlayer extends Component {
  ...
  componentDidMount() {
    ...
    this.setState({ backnfortharray: a }, function () {
      this.context.UpdateCurrentContentDetails(
        this.state.backnfortharray[this.props.id - 1]
      );
    });
  }
  ...
  handleNext() {
    if (
      this.context.CurrentContentDetails.id <=
        this.state.backnfortharray.length - 1 &&
      parseInt(this.context.currentCourseProgress.completedItem) >=
      ...
      this.context.UpdateCurrentContentDetails(
        this.state.backnfortharray[this.props.id]
      );
      this.props.history.push(
        `/course/research-methodology/${
          this.state.backnfortharray[this.props.id].id
        }`
      );
  ...
  handlePrev() {
    if (this.context.CurrentContentDetails.id - 2 >= 0) {
      ...
      if (this.props.Isopen) {
        const node = ReactDOM.findDOMNode(this.props.rf.current);
        node.scrollTop =
          node.scrollTop - this.context.CurrentContentDetails.id * 10;
        console.log(node.scrollTop);
      }
      this.context.UpdateCurrentContentDetails(
        this.state.backnfortharray[this.props.id - 2]
      );
      this.props.history.push(
        `/course/research-methodology/${
          this.state.backnfortharray[this.props.id - 2].id
        }`
      );
      // window.location.reload()
    }
  ...
  handleupload() {
    ...
    axios
      .post(
        `https://nodeapi.beresearcherbd.com/api/upload?id=${this.context.CurrentUserDetails.id}`,
        data,
        options
      )
      .then((res) => {
        ...
        if (
          ...
          setTimeout(() => {
            axios({
              method: 'PUT',
              url:
                'https://beresearcherbd.herokuapp.com/api//student/courseprogress',
              headers: {
              ...
              data: JSON.stringify({
                ...
                currentContentDetails:
                  parseInt(this.state.backnfortharray.length) - 1 >=
                  parseInt(this.props.id)
                    ? this.state.backnfortharray[this.props.id]
                    : this.state.backnfortharray[this.props.id - 1],
              }),
    ...
    return (
      <div className="VideoContainer">
        ...
        <div className="forwardbackward">
          ...
          <span
            onClick={this.handleMarkasDone.bind(this)}
            className="btn2"
            style={this.state.donebuttonstyle}
          >
            Mark as Done
            <FontAwesomeIcon
              id="tiksign"
